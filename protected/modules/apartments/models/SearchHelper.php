    ...
    public static function genCriteria($model, CDbCriteria $criteria, $searchType = 'search')
    {
        ...
        if ($model->bStart) {
            if (param('booking_half_day')) {
                $dateStart = Yii::app()->dateFormatter->format('yyyy-MM-dd', CDateTimeParser::parse($model->bStart, Booking::getYiiDateFormat()) + 86400);
            } else {
                $dateStart = Yii::app()->dateFormatter->format('yyyy-MM-dd', CDateTimeParser::parse($model->bStart, Booking::getYiiDateFormat()));
            }
            if ($model->bEnd) {
                if (param('booking_half_day')) {
                    $dateEnd = Yii::app()->dateFormatter->format('yyyy-MM-dd', CDateTimeParser::parse($model->bEnd, Booking::getYiiDateFormat()) - 86400);
                } else {
