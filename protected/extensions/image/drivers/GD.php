...
class Image_GD_Driver extends Image_Driver {
	...
	{	
		...
		if ($status = imagecopyresampled($img, $this->tmp_image, round(($properties['width']-$new_width)/2), round(($properties['height']-$new_height)/2),  0,  0, round($new_width), round($new_height), round($src_width), round($src_height)))
		{
			// Swap the new image for the old one
			imagedestroy($this->tmp_image);
			$this->tmp_image = $img;
